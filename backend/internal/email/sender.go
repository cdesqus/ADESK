...
func (sc *SMTPClient) SendAutoReply(toEmail, toName, subject string, ticketID uint, customer *models.Customer, emailBody, openAIKey string) error {
	...
	if openAIKey != "" {
		...
		aiBody, err := aiClient.GenerateAutoReply(emailBody, customerName, sc.fromName, ticketID)
		if err == nil && aiBody != "" {
			body = aiBody
		} else {
			log.Printf("AI auto-reply failed, falling back to default template: %v", err)
		}
	}
