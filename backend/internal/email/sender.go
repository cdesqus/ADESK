...
func (sc *SMTPClient) SendAutoReply(toEmail, toName, subject string, ticketID uint, customer *models.Customer, emailBody, openAIKey string) error {
	...
	if openAIKey != "" {
		...
		if err == nil && aiBody != "" {
			body = aiBody + "\n\n---\nThis is an automated response generated by AI. Please do not reply to this email."
		} else {
			log.Printf("AI auto-reply failed, falling back to default: %v", err)
			body = body + fmt.Sprintf("\n\n[SYSTEM DEBUG: AI Generation Failed: %v]\n[SYSTEM DEBUG: Key Length: %d]", err, len(openAIKey))
		}
	} else {
		body = body + "\n\n[SYSTEM DEBUG: OPENAI_API_KEY is empty in the environment!]"
	}
