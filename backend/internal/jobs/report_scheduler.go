...
func NewReportScheduler(db *gorm.DB, generator *reports.ReportGenerator,
	mailer *reports.ReportMailer, repo *reports.ReportRepository, slaHours int) *ReportScheduler {
...
func (rs *ReportScheduler) generateAndSendReport(ctx context.Context, customer models.Customer, month, year int) error {
	...
	// Export to CSV
	csvBytes, err := reports.ExportToCSV(reportData)
	if err != nil {
		return fmt.Errorf("failed to export CSV: %w", err)
	}

	// Export to PDF
	pdfBytes, err := reports.ExportToPDF(reportData)
	if err != nil {
